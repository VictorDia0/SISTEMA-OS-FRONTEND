...
export default function DocenteListPage() {
  ...
  useEffect(() => {
    async function carregarDocentes() {
      try {
        ...
        console.error("Erro ao buscar docentes");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Deseja realmente excluir este docente?")) return;
    try {
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-white/70">
              Gerencie todos os docentes do sistema acadêmico
            </p>
          </div>
          <Button className="bg-white/20 hover:bg-white/30 text-white border border-white/30">
            <Link href="/admin/docentes/novo" className="flex items-center gap-2">
        ...
        <Card className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-lg p-6 mb-8">
          {loading ? (
            <div className="text-center py-8">
              ...
              <p className="text-white mt-4">Carregando docentes...</p>
            </div>
          ) : docentes.length === 0 ? (
            <div className="text-center py-12">
              <Users className="h-16 w-16 text-white/50 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-white mb-2">Nenhum docente cadastrado</h3>
              <p className="text-white/70 mb-6">Comece adicionando o primeiro docente ao sistema</p>
              <Button className="bg-white/20 hover:bg-white/30 text-white border border-white/30">
                <Link href="/admin/docentes/novo" className="flex items-center gap-2">
                  <Plus className="h-4 w-4" />
                  Adicionar Docente
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/20">
                    ...
                    <th className="p-4 text-left text-white font-semibold">Área</th>
                    <th className="p-4 text-left text-white font-semibold">Titulação</th>
                    <th className="p-4 text-left text-white font-semibold">Data de Contratação</th>
                    <th className="p-4 text-center text-white font-semibold">Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {docentes.map((docente) => (
                    <tr key={docente.id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                      <td className="p-4 text-white">{docente.id}</td>
                      <td className="p-4 text-white">{docente.nome}</td>
                      <td className="p-4 text-white/80">{docente.email}</td>
                      <td className="p-4 text-white">{docente.area}</td>
                      <td className="p-4 text-white">{docente.titulacao}</td>
                      <td className="p-4 text-white">{formatarData(docente.dataContratacao)}</td>
