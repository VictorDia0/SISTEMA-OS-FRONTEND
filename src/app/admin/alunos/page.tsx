...
export default function AlunoListPage() {
  ...
  const [editForm, setEditForm] = useState<AlunoUpdateDTO>({});
  ...
  const handleEditChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEditForm(prev => ({
      ...prev,
      [name]: name === 'semestre' || name === 'ano' ? Number(value) : value
    }));
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setEditLoading(true);
    try {
      const response = await api.put(`/api/alunos/${editingAluno.matricula}`, editForm);

      setAlunos(alunos.map(aluno =>
        aluno.matricula === editingAluno.matricula
          ? { ...aluno, ...response.data }
          : aluno
      ));
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900">
        ...
        {editingAluno && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800 rounded-2xl shadow-xl w-full max-w-md border border-white/20">
